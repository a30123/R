...
while(i<no_of_runs){  
  ...
  if(length(Bakelistleft)>0){
    rect(Bakelistleft,rep(lower_plot_limit,length(Bakelistleft)),Bakelistright,rep(upper_plot_limit,length(Bakelistleft)),col=adjustcolor("mistyrose",alpha=0.5),border=NA)
  }
  
  if(length(HClBakelistleft)>0){
    rect(HClBakelistleft,rep(lower_plot_limit,length(HClBakelistleft)),HClBakelistright,rep(upper_plot_limit,length(HClBakelistleft)),col=adjustcolor("pink",alpha=0.5),border=NA)
  }
  
  if(length(Bufferlistleft)>0){
    rect(Bufferlistleft,rep(lower_plot_limit,length(Bufferlistleft)),Bufferlistright,rep(upper_plot_limit,length(Bufferlistleft)),col=adjustcolor("skyblue",alpha=0.5),border=NA)
  }
